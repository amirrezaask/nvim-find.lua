...
function M.floating_fuzzy(opts)
	...
	local get_qf_entry = opts.get_qf_entry
		or function(e)
			return {
				{
	...
	local include_scores = opts.include_scores ~= nil
	local frame_source = {}
	...
	local function update()
		...
		if prev ~= user_input or (prev == "" and user_input == "") then
			if opts.live then
				opts[1](function(e)
					update_source(e)
				end, user_input)
	...
	local function accept()
		if not source[selected_item + 1] then
			return
		end
...
function M.files(opts)
	...
	local function luv_find(opts)
		...
		if opts.starting_directory == nil then
			opts.starting_directory = opts.path
		end
...
function M.ripgrep_fuzzy(opts)
	...
	vim.ui.input({ prompt = opts.prompt or "Fuzzy Ripgrep> " }, function(s)
		...
		opts[1] = read_output_by_line(
			"rg",
			{ "--color", "never", "--column", "-n", "--no-heading", s },
			opts.cwd,
			function(line)
				if line == "" then
					return {}
...
function M.git_files(opts)
	...
	opts[1] = read_output_by_line("git", { "ls-files" }, opts.cwd, function(line)
		local path = line
		-- if opts.shorten_paths then
...
local function make_display_from_lsp_result(filename, sym, line)
	return string.format("[%s] %s", sym.kind, sym.name)
end
